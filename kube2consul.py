...
Usage:
  kube2pyconsul.py [-v <loglevel>] [--verify-ssl=<value>] [--consul-agent=<consul-uri>] [--kube-master=<kubeapi-uri>] [--consul-auth=<user,pass>] [--kube-auth=<u
ser,pass>]
                ...
                if event['type'] == 'ADDED':
                    ...
                    for ports in portlist:
                        ...
                        if r.status_code == 200:
                            ...
                            log.error("Consul returned non-200 request status code. Could not register service {}. Continuing on to the next service...".format(s
ervice))
                        sys.stdout.flush()
...
ort']), 
                        ...
                        if r.status_code == 200:
                            ...
                            log.error("Consul returned non-200 request status code. Could not deregister service {service}. Continuing on to the next service..."
.format())
                        sys.stdout.flush()
